...
namespace FolderView
{
    ...
    {
        /*
            PVS.MediaPlayer 1.4 Example Application - FolderView
            ...
            the PVS.MediaPlayer library version 1.4 - licensed under The Code Project Open License (CPOL)
        ...
        */
        ...
        // Movie/image file extensions to look for (this list starts with .lnk (shortcut file))
        private const string    MOVIE_EXTENSIONS = ".lnk.3g2.3gp.3gp2.3gpp.asf.avi.hevc.m2ts.m4v.mkv.mov.mp4.mpeg.mpg.mts.sami.vob.webm.wmv.bmp.gif.heic.jfif.jpeg.jpg.png.tif.tiff";
        ...
        public MainWindow()
        {
            ...
            _backPlayer = new Player(flowLayoutPanel1);
            _backPlayer.DragAndDrop.Add(this);
        ...
        private void CreateInfoLabels()
        {
            ...
            ItemViewLabel.BackBrush         = new LinearGradientBrush(new Rectangle(new Point(0, 0), ItemViewLabel.Size), Color.FromArgb(80, 80, 80), Color.Black, LinearGradientMode.Vertical);
            ...
            PlayerWindowLabel.BackBrush     = new LinearGradientBrush( new Rectangle(new Point(0, 0), PlayerWindowLabel.Size), Color.FromArgb(64, 64, 64), Color.Black, LinearGradientMode.Vertical);
            ...
            do
            {
                ...
                if (_fileList.Count > 0)
                {
                    ...
                    for (int i = 0; i < _playerCount && !_hasQueue; i++)
                    {
                        ...
                        _myPlayers[i].Images.Duration               = 1;
                        //_myPlayers[i].Images.FrameRate              = 4; // 4 too low, problems with redraw
        ...
        private void RoundedShapeAllMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _myPlayers.Length; i++)
            {
                if (_myPlayers[i] != null)
                {
                    _myPlayers[i].Display.Shape = DisplayShape.Rounded;
                    ((Panel)_myPlayers[i].Display.Window).BorderStyle = BorderStyle.None;
                }
            }
        }
        ...
        private void NormalShapeAllMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _myPlayers.Length; i++)
            {
                if (_myPlayers[i] != null)
                {
                    _myPlayers[i].Display.Shape = DisplayShape.Normal;
                    ((Panel)_myPlayers[i].Display.Window).BorderStyle = BorderStyle.FixedSingle;
                }
            }
        }
        ...
        private void OpenItem(bool fromStart)
        {
            ...
            if (_myPlayers[_playerId] != null)
            {
                ...
                else
                {
                    ...
                    if (Control.ModifierKeys == Keys.Control) playerForm.WindowState = FormWindowState.Maximized;
        ...
        // Size To Fit all Player Windows
        internal void SizeToFitAllWindows()
        {
            foreach (Form window in Application.OpenForms)
            {
                if (window != this && window.GetType() == typeof(PlayerWindow)) ((PlayerWindow)window).SizeToFitMenuFromMain();
            }
        }
